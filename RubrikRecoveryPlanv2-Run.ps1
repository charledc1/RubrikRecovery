...
foreach ($VM in $RecoveryPlanVMs)
{
  ...
  IF ($VMSnapshotID -ne $null)
  {
    ...
    ################################################
    # Getting Job and export status link to a file
    ################################################
    # Getting Status URL
    ...
    $ExportobStatusCount ++
      ...
      Try 
      {
        ...
        # Setting status link to text file "Rubrik-Query-Thursday.txt
        $ExportJobStatusQuery = $ExportJobStatusResponse.links.href
        $ExportJobStatusQuery  | Out-File -Encoding Ascii -Append -FilePath C:\RubrikRecoveryPlanv2\Advanced\Logs\Rubrik-Query-$Day.txt
      }
      Catch 
      {
        $_.Exception.ToString()
